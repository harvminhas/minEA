...
const LAYOUT_OPTIONS: { id: ApiViewLayout; label: string; icon: typeof LayoutGrid }[] = [
...
export function ApiList() {
  ...
  const [viewLayout, setViewLayout] = useState<ApiViewLayout>("table");
  ...
  const filterOpts = useMemo(() => apiFilterOptions(items), [items]);
  ...
  const filtered = useMemo(
    ...
    [items, search, styleFilter, statusFilter, ownerFilter]
  );
  ...
  const exportCsv = () => {
    ...
    const rows = filtered.map((item) => [
      ...
      String(apiConsumerCount(item)),
      apiAuthLabel(item),
      apiCriticalityLabel(item),
      item.owner ?? "",
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Integration layer · {data?.total ?? items.length} API
        ...
        >
          ...
          <FilterDropdown
            ...
            options={[
              ...
              ...filterOpts.styles.map((s) => ({
                value: s,
                label: API_STYLE_LABEL[s] ?? s,
              })),
            ]}
          />
          <FilterDropdown
      ...
      <div className="flex-1 overflow-y-auto p-8">
        {listLoading ? (
          ...
          >
            ...
            <p className="mt-3 text-sm text-gray-500">Loading APIs…</p>
          </div>
        ) : viewLayout === "table" ? (
          filtered.length === 0 ? (
            <div className="text-center py-16 rounded-xl border border-dashed border-gray-200 bg-white/60">
              <p className="text-gray-500 text-sm mb-3">
                {items.length === 0 ? "No APIs documented yet." : "No APIs match your filters."}
              </p>
              {canCreate && (
                <button
                  type="button"
