...
export function RegisterGatewayDialog({ defaultPlatform, onClose, onCreated }: Props) {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      return objectsApi.create(
        ...
        workspaceSlug,
        {
          ...
          name: trimmed,
          status: "active",
          properties: { gateway_platform: platform },
        },
        token
      );
    },
    ...
    onError: (err) =>
      setError(err instanceof Error ? err.message : "Could not register gateway"),
  });
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[230] w-full max-w-sm bg-white rounded-xl shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Register gateway</h3>
            <p className="text-xs text-gray-400 mt-0.5">Add a workspace API gateway instance</p>
          </div>
