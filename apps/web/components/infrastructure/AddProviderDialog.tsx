...
interface Props {
  onClose: () => void;
  onAdded: (providerName: string) => void;
}

export function AddProviderDialog({ onClose, onAdded }: Props) {
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[230] w-full max-w-sm bg-white rounded-xl shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Add provider</h3>
            <p className="text-xs text-gray-400 mt-0.5">Register a cloud or hosting provider</p>
          </div>
