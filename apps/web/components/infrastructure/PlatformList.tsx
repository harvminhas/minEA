...
function PlatformCard({ item, onOpenDetail }: { item: MinEAObject; onOpenDetail: () => void }) {
  ...
  const lifecycle = props.lifecycle
    ? PLATFORM_LIFECYCLE_LABEL[props.lifecycle]
    : undefined;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-1 flex-wrap">
            ...
            {vendorLabel && (
              <span className="text-[10px] text-gray-500">{vendorLabel}</span>
            )}
            {props.criticality && (
              <span
                className={cn(
...
export function PlatformList() {
  ...
  const platformsQueryKey = ["objects", orgSlug, workspaceSlug, "cloud_service"] as const;

  const { data, isLoading } = useQuery({
    queryKey: platformsQueryKey,
    queryFn: async () => {
      const token = await getToken();
      return objectsApi.list(orgSlug, workspaceSlug, { type: "cloud_service" }, token!);
    },
    enabled,
  });
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-8 py-4 border-b border-gray-200 bg-white sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold text-gray-900">Platforms</h1>
          {data && (
      ...
      <div className="flex-1 overflow-y-auto p-8">
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {items.map((item) => (
              <PlatformCard key={item.id} item={item} onOpenDetail={() => setSelectedId(item.id)} />
            ))}
          </div>
        )}
      </div>
