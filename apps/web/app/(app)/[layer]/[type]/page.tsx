...
import { OBJECT_TYPE_LABELS, LAYER_CONFIG, type ObjectType, type MinEAObject } from "@minea/types";
import { getLayerColor } from "@/lib/utils";

// Map URL path segments to object types
...
export default function ObjectListPage({ params }: { params: Promise<{ layer: string; type: string }> }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["objects", activeWorkspace?.id, objectType],
    enabled: !!activeWorkspace,
    queryFn: async () => {
      const token = await getToken();
      return objectsApi.list({ workspace_id: activeWorkspace!.id, type: objectType }, token!);
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const token = await getToken();
      return objectsApi.delete(id, token!);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["objects", activeWorkspace?.id, objectType] });
      ...
      <div className="flex-1 overflow-y-auto p-8">
        {!activeWorkspace ? (
          <div className="text-center py-16 text-gray-400">
            <p className="text-sm">Select a workspace to view objects.</p>
          </div>
        ) : isLoading ? (
